...
create trigger updateBalance_accountExpense
  ...
  for each row
  update balance
  set dollar_in = dollar_in + new.dollar, hr_in = hr_in + new.hr
  where who = new.firmaName;
...
create trigger updateBalance
  ...
  for each row
  update balance
  set sum_in = sum_in + new.sum, dollar_in = dollar_in + new.dollar, hr_in = hr_in + new.hr,
    sum_out  = sum_out + new.psum, dollar_out = dollar_out + new.pdollar
    , hr_out = hr_out + new.phr
...
create trigger revertBalance
  ...
  update balance
  set sum_in = sum_in - new.sum, dollar_in = dollar_in - new.dollar, hr_in = hr_in - new.hr,
    sum_out  = sum_out - new.psum, dollar_out = dollar_out - new.pdollar, hr_out = hr_out - new.phr
  where who = new.customer_id;
...
create view accountexpense_v as
  select
    `a`.`id`            AS `id`,
    `a`.`schot`         AS `schot`,
...
create view admin_history_v as
  select
  ...
  where (`sbd_factory`.`log`.`module` = 'Admin');

create view balance_v as
  select
    `b`.`id`                             AS `id`,
...
create view main1_history_v as
  select
    `sbd_factory`.`log`.`id`        AS `id`,
    `sbd_factory`.`log`.`module`    AS `module`,
    `sbd_factory`.`log`.`type`      AS `type`,
...
create view main2_history_v as
  select
    `sbd_factory`.`log`.`id`        AS `id`,
    `sbd_factory`.`log`.`module`    AS `module`,
    `sbd_factory`.`log`.`type`      AS `type`,
...
create view product_history_v as
  select
    `sbd_factory`.`log`.`id`        AS `id`,
    `sbd_factory`.`log`.`module`    AS `module`,
    `sbd_factory`.`log`.`type`      AS `type`,
...
create view savdo_history_v as
  select
    `sbd_factory`.`log`.`id`        AS `id`,
    `sbd_factory`.`log`.`module`    AS `module`,
    `sbd_factory`.`log`.`type`      AS `type`,
  ...
  select
    ...
    (case `u`.`userType`
     ...
     when '4'
       then 'Savdo'
     when '5'
       then 'Admin' end) AS `userType`
  from `sbd_factory`.`user` `u`
  order by `u`.`id`;

create procedure addSuplier(IN companyName varchar(100))
  BEGIN
    insert into person (type, companyName, account, phone, info, cr_by, date_cr)
    VALUES (1, companyName, 1, 1, 1, 1, sysdate());
  END;
