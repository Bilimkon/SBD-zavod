create schema if not exists sbd_factory collate utf8mb4_0900_ai_ci;

create table if not exists accountexpense
(
...
create definer = root@localhost trigger insert_omborReport1
	after insert
	on accountexpense
	for each row
...
create definer = root@localhost trigger updateBalance_accountExpense
	after insert
	on accountexpense
	for each row
	update balance set dollar_in = dollar_in+ new.dollar, hr_in = hr_in + new.hr where who = new.firmaName;

create definer = root@localhost trigger updateTotalBalance_accountExpense12
	after insert
	on accountexpense
	for each row
...
create definer = root@localhost trigger admin_oper_update_total_balance_r
	after insert
	on admin_oper_r
	for each row
...
create definer = root@localhost trigger admin_oper_update_who_balance_r
	after insert
	on admin_oper_r
	for each row
	if new.type='Kirim' then
...
create table if not exists adminoper
(
	id int auto_increment
		primary key,
	type varchar(100) null,
...
create definer = root@localhost trigger admin_oper_update_total_balance
	after insert
	on adminoper
	for each row
...
create definer = root@localhost trigger admin_oper_update_who_balance
	after insert
	on adminoper
	for each row
	if new.type='Chiqim' then
...
create table if not exists dollarexchange
(
	id int auto_increment
		primary key,
	hr decimal(20,2) null,
...
create definer = root@localhost trigger insertReport1_history
	after insert
	on history
	for each row
	update report1
...
create definer = root@localhost trigger update_sell
	after insert
	on history
	for each row
	update sell
...
create table if not exists history_r
(
	id int auto_increment
		primary key,
	barcode varchar(30) null,
...
create definer = root@localhost trigger revertPerProduct
	after insert
	on history_r
	for each row
...
create definer = root@localhost trigger insert_omborReport
	after insert
	on invoice
	for each row
...
create table if not exists manlist
(
	id int auto_increment
		primary key,
	name varchar(200) null,
...
create table if not exists marketing_filter
(
	id int auto_increment
		primary key,
	company int null,
...
create table if not exists p2list
(
	id int auto_increment
		primary key,
	name varchar(200) null,
...
create definer = root@localhost trigger addCustomer
	after insert
	on person
	for each row
...
create definer = root@localhost trigger insertOmborReport2
	after insert
	on product_h
	for each row
	update reportOmbor
...
create table if not exists production2
(
	id int auto_increment
		primary key,
	barcode varchar(40) null,
...
create table if not exists production2_ready
(
	id int auto_increment
		primary key,
	barcode varchar(40) null,
...
create table if not exists production2_tarix
(
	id int auto_increment
		primary key,
	barcode varchar(40) null,
...
create table if not exists production2_xarajat
(
	id int auto_increment
		primary key,
	barcode varchar(100) null,
...
create table if not exists production3
(
	id int auto_increment
		primary key,
...
create table if not exists reportombor
(
	id int auto_increment,
	type varchar(200) null,
...
alter table reportombor
	add primary key (id);

create table if not exists sale_balance
(
	id int auto_increment
		primary key,
	type varchar(100) null,
...
create definer = root@localhost trigger insert_report1_saleBalance
	after insert
	on sale_balance
	for each row
...
create definer = root@localhost trigger sale_balance_update_who_balance
	after insert
	on sale_balance
	for each row
	if new.type='Chiqim' then
...
create table if not exists sellaction
(
	id int auto_increment
		primary key,
	sum decimal(20,2) null,
...
create definer = root@localhost trigger insert_report1_sellAction
	after insert
	on sellaction
	for each row
...
create definer = root@localhost trigger updateBalance
	after insert
	on sellaction
	for each row
...
create table if not exists sellaction_r
(
	id int auto_increment
		primary key,
	sum decimal(20,2) null,
...
create definer = root@localhost trigger revertBalance
	after insert
	on sellaction_r
	for each row
...
create table if not exists sellupdatelog
(
	id int auto_increment
		primary key,
	name varchar(100) null,
...
create table if not exists t_balance_rate
(
	id int auto_increment
		primary key,
	cr_on varchar(100) null,
...
create or replace definer = root@localhost view accountexpense_v as select `a`.`id`            AS `id`,
       `a`.`schot`         AS `schot`,
...
create or replace definer = root@localhost view admin_history_v as select `sbd_factory`.`log`.`id`        AS `id`,
...
create or replace definer = root@localhost view balance_v as select `b`.`id`                             AS `id`,
...
create or replace definer = root@localhost view balancetotal_v as select (select sum((`sbd_factory`.`product`.`quantity` * `sbd_factory`.`product`.`cost`)) AS `total_cost`
       ...
       (select `sbd_factory`.`total_balance`.`sum` from `sbd_factory`.`total_balance`)    AS `sum`,
       (select `sbd_factory`.`total_balance`.`dollar` from `sbd_factory`.`total_balance`) AS `dollar`,
       (select `sbd_factory`.`total_balance`.`hr` from `sbd_factory`.`total_balance`)     AS `hr`,
       (select `sbd_factory`.`total_balance`.`vhr` from `sbd_factory`.`total_balance`)    AS `vhr`,
       (((((select sum((`sbd_factory`.`product`.`quantity` * `sbd_factory`.`product`.`cost`)) AS `total_cost`
            from `sbd_factory`.`product`) +
           (select sum((`sbd_factory`.`production2_ready`.`quantity` * 5)) AS `total_p1`
            from `sbd_factory`.`production2_ready`)) +
          (select sum((`sbd_factory`.`sell`.`quantity` * `sbd_factory`.`sell`.`cost`)) AS `total_sell`
           from `sbd_factory`.`sell`)) +
         (select `sbd_factory`.`total_balance`.`dollar` from `sbd_factory`.`total_balance`)) +
        (select `sbd_factory`.`total_balance`.`vhr` from `sbd_factory`.`total_balance`))  AS `total_all`;

create or replace definer = root@localhost view debtors_v as select `b`.`id`                             AS `id`,
       `p`.`companyName`                    AS `who`,
       `b`.`sum_in`                         AS `sum_in`,
       `b`.`sum_out`                        AS `sum_out`,
       (`b`.`sum_out` - `b`.`sum_in`)       AS `sum_balance`,
...
create or replace definer = root@localhost view exchange_v_s as select `e`.`id`       AS `id`,
       `e`.`cr_on`    AS `cr_on`,
       `e`.`Name`     AS `name`,
       ...
       `u`.`username` AS `cr_by`
from ((`sbd_factory`.`exchange_log` `e` join `sbd_factory`.`user` `u`)
         join `sbd_factory`.`type` `t` on (((`e`.`cr_by` = `u`.`id`) and (`t`.`id` = `e`.`type`))))
where ((`e`.`comment` = '1-S') or (`e`.`comment` = 'O-S') or (`e`.`comment` = '2-S'));

create or replace definer = root@localhost view history_v as select `h`.`id`                      AS `id`,
...
from ((`sbd_factory`.`history` `h` join `sbd_factory`.`user` `u`)
         join `sbd_factory`.`person` `p` on (((`h`.`seller_id` = `u`.`id`) and (`h`.`customer_id` = `p`.`id`))));

create or replace definer = root@localhost view log_v as select `sbd_factory`.`log`.`id`        AS `id`,
       `sbd_factory`.`log`.`module`    AS `module`,
       `sbd_factory`.`log`.`type`      AS `type`,
       `sbd_factory`.`log`.`cost`      AS `cost`,
       `sbd_factory`.`user`.`username` AS `cr_by`,
...
create or replace definer = root@localhost view main1_history_v as select `sbd_factory`.`log`.`id`        AS `id`,
       `sbd_factory`.`log`.`module`    AS `module`,
       `sbd_factory`.`log`.`type`      AS `type`,
       `sbd_factory`.`log`.`cost`      AS `cost`,
       `sbd_factory`.`user`.`username` AS `cr_by`,
...
create or replace definer = root@localhost view main2_history_v as select `sbd_factory`.`log`.`id`        AS `id`,
       `sbd_factory`.`log`.`module`    AS `module`,
       `sbd_factory`.`log`.`type`      AS `type`,
       `sbd_factory`.`log`.`cost`      AS `cost`,
       `sbd_factory`.`user`.`username` AS `cr_by`,
...
create or replace definer = root@localhost view manlist_v as select `m`.`id`          AS `id`,
       `m`.`name`        AS `name`,
...
create or replace definer = root@localhost view marketing_v as select `m`.`id`          AS `id`,
...
create or replace definer = root@localhost view product_h_v as select `p`.`id`                      AS `id`,
       ...
       `u`.`username`                AS `user`
from ((((`sbd_factory`.`product_h` `p` join `sbd_factory`.`person` `s`) join `sbd_factory`.`user` `u`) join `sbd_factory`.`invoice` `i`)
         join `sbd_factory`.`color` `c`
              on (((`p`.`cr_by` = `u`.`id`) and (`p`.`suplier_id` = `s`.`id`) and (`p`.`invoice_id` = `i`.`id`))));

create or replace definer = root@localhost view product_history_v as select `sbd_factory`.`log`.`id`        AS `id`,
       `sbd_factory`.`log`.`module`    AS `module`,
       `sbd_factory`.`log`.`type`      AS `type`,
       `sbd_factory`.`log`.`cost`      AS `cost`,
       `sbd_factory`.`user`.`username` AS `cr_by`,
...
create or replace definer = root@localhost view product_v as select `p`.`id`                      AS `id`,
       `i`.`name`                    AS `invoice`,
       (case `p`.`unit`
            when '1' then 'Dona'
            when '2' then 'Kg'
       ...
       `p`.`date_cr`                 AS `date_cr`,
       `u`.`username`                AS `user`
from ((((`sbd_factory`.`product` `p` join `sbd_factory`.`person` `s`) join `sbd_factory`.`user` `u`) join `sbd_factory`.`invoice` `i`)
         join `sbd_factory`.`color` `c`
              on (((`p`.`cr_by` = `u`.`id`) and (`p`.`suplier_id` = `s`.`id`) and (`p`.`invoice_id` = `i`.`id`))));

create or replace definer = root@localhost view production2_v as select `p`.`id`         AS `id`,
       `p`.`barcode`    AS `barcode`,
       `p`.`name`       AS `name`,
       `p`.`type`       AS `type`,
       `p`.`quantity`   AS `quantity`,
...
create or replace definer = root@localhost view production3_v as select `p`.`id`        AS `id`,
       `p`.`barcode`   AS `barcode`,
       `p`.`name`      AS `name`,
       `p`.`type`      AS `type`,
       `p`.`color`     AS `color`,
...
create or replace definer = root@localhost view report1_v as select `r`.`id`          AS `id`,
       `r`.`type`        AS `type`,
       `p`.`companyName` AS `who`,
       `r`.`sum`         AS `sum`,
       `r`.`dollar`      AS `dollar`,
...
create or replace definer = root@localhost view sale_balance_v as select `s`.`id`          AS `id`,
       `s`.`type`        AS `type`,
       `p`.`companyName` AS `who`,
       `s`.`sum`         AS `sum`,
       `s`.`dollar`      AS `dollar`,
...
from ((`sbd_factory`.`sale_balance` `s` join `sbd_factory`.`person` `p`)
         join `sbd_factory`.`user` `u` on (((`s`.`who` = `p`.`id`) and (`s`.`cr_by` = `u`.`id`))));

create or replace definer = root@localhost view savdo_history_v as select `sbd_factory`.`log`.`id`        AS `id`,
       `sbd_factory`.`log`.`module`    AS `module`,
       `sbd_factory`.`log`.`type`      AS `type`,
       `sbd_factory`.`log`.`cost`      AS `cost`,
       `sbd_factory`.`user`.`username` AS `cr_by`,
...
create or replace definer = root@localhost view sell_v as select `s`.`id`                     AS `id`,
       ...
       `s`.`description`            AS `description`
from ((`sbd_factory`.`sell` `s` join `sbd_factory`.`type` `t`)
         join `sbd_factory`.`user` `u` on (((`s`.`type_id` = `t`.`id`) and (`s`.`cr_by` = `u`.`id`))));

create or replace definer = root@localhost view sellaction_v as select `s`.`id`          AS `id`,
       `s`.`sum`         AS `sum`,
       `s`.`dollar`      AS `dollar`,
       `s`.`hr`          AS `hr`,
       `s`.`psum`        AS `psum`,
...
from ((`sbd_factory`.`sellaction` `s` join `sbd_factory`.`person` `p`)
         join `sbd_factory`.`user` `u` on (((`s`.`customer_id` = `p`.`id`) and (`s`.`cr_by` = `u`.`id`))));

create or replace definer = root@localhost view total_balance_v as select (select sum((`sbd_factory`.`product`.`cost` * `sbd_factory`.`product`.`quantity`))
...
create or replace definer = root@localhost view type_v as select `t`.`id`                     AS `id`,
       `t`.`name`                   AS `name`,
       `t`.`info`                   AS `info`,
...
create or replace definer = root@localhost view user_v as select `u`.`id`                             AS `id`,
       `u`.`username`                       AS `username`,
       ...
       (case `u`.`userType`
            ...
            when '4' then 'Savdo'
            when '5' then 'Admin'
            when '6' then 'Accounting' end) AS `userType`
from `sbd_factory`.`user` `u`
order by `u`.`id`;

create definer = root@localhost procedure addSuplier(IN companyName varchar(100))
BEGIN
