...
public class Admin1 implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        ExchangeCombobox.getItems().addAll("Sum-Dollar", "HR-Dollar", "Dollar-Sum");
        initializeTable();
    ...
    public void btnExchangeAction() throws SQLException {
        ...
        try {
            ...
            int sum = Integer.parseInt(ExchangeSumma.getText().trim().replaceAll("\\s+", ""));
            int dollar = Integer.parseInt(DollarRate.getText().trim().replaceAll("\\s+", ""));

            if (oper.equals("Sum-Dollar")) {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                ...
                if (result.isPresent())
                    if (result.get() == ButtonType.OK) {
                        ...
                        pr = myConn.prepareStatement("update total_balance set dollar=(dollar+?)");
                        pr.setString(1, String.valueOf((sum / dollar)));
                        pr.executeUpdate();
                        pr1 = myConn.prepareStatement("update total_balance set sum=(sum-?)");
                ...
                if (result.isPresent())
                    if (result.get() == ButtonType.OK) {
                        ...
                        pr = myConn.prepareStatement("update total_balance set dollar=(dollar+?)");
                        pr.setInt(1,(sum / dollar));
                        pr.executeUpdate();
                        pr1 = myConn.prepareStatement("update total_balance set hr=(hr-?)");
                        pr1.setString(1, String.valueOf(sum));
                        pr1.executeUpdate();
                        total_Sum();
            ...
            } else if (oper.equals("Dollar-Sum")) {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                ...
                if (result.isPresent())
                    if (result.get() == ButtonType.OK) {
                        ...
                        pr = myConn.prepareStatement("update total_balance set sum=(sum+?)");
                        pr.setString(1, String.valueOf((dollar * sum)));
                        pr.executeUpdate();
                        pr1 = myConn.prepareStatement("update total_balance set dollar=(dollar-?)");
                        pr1.setString(1, String.valueOf(sum));
                        pr1.executeUpdate();
                        total_Sum();
