...
public class AdminDao {
    ...
    public void total_balance(Label sum, Label dollar, Label account, Label VAL) {
        ...
        try {
            statement = myConn.createStatement();
            resultSet = statement.executeQuery("Select * from total_balance");
            while (resultSet.next()) {
                sum.setText(resultSet.getString("sum"));
                dollar.setText(resultSet.getString("dollar"));
                account.setText(resultSet.getString("hr"));
                VAL.setText(resultSet.getString("vhr"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public void chart(String dan, String gacha, AreaChart areaChart) throws SQLException {
        ...
        try {
            ...
            if (!dan.equals("1") && !gacha.equals("1")) {
                ...
                statement = myConn.createStatement();
                resultSet = statement.executeQuery("SELECT * FROM history ORDER BY id limit 100");
            }
